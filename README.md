...
CData's Model Context Protocol (MCP) Server for Sage 300

:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Sage 300 (beta)](https://www.cdata.com/download/download.aspx?sku=GTZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Sage 300 supported by the [CData JDBC Driver for Sage 300](https://www.cdata.com/drivers/sage300/jdbc).

CData JDBC Driver connects to Sage 300 by exposing them as relational SQL models.

This server wraps that driver and makes Sage 300 data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/sage300/download/jdbc](https://www.cdata.com/drivers/sage300/download/jdbc)
3. License the CData JDBC Driver:
    ...
    * Run the command `java -jar cdata.jdbc.sage300.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.sage300.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `sage-300.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.sage300.Sage300Driver)
